...
public class FrmBillManager extends JFrame implements ActionListener, MouseListener {
	...
	public FrmBillManager() {
		...
		lblMaNV.setBounds(238, 140, 454, 35);
		pnlInfo.add(lblMaNV);
	...
	private void updateTableBill() throws RemoteException {
		...
		for (Bill bill : bills) {
			String rowBill[] = { bill.getIdBill(), bill.getEmployee().getIdEmloyee(), bill.getNameCustomer(),
					bill.getDatecreate() + "", bill.getMoney() + "" };
			data_ModelBill.addRow(rowBill);;
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (object.equals(table_Bill)) {
			...
			lblMaNV.setText(table_Bill.getValueAt(row, 1).toString());
			txtTenKH.setText(table_Bill.getValueAt(row, 2).toString());
			lblTongTien.setText(table_Bill.getValueAt(row, 4).toString());
			try {
				SeachProductBuyForIDBill(row);
			} catch (RemoteException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	...
	private void SeachProductBuyForIDBill(int row) throws RemoteException{
		productBuys.clear();
		productBuys = productBuyDao.getProductBuyOfIdBill(table_Bill.getValueAt(row, 0).toString());
		DeleteTableProductBuy();
		for (ProductBuy p : productBuys) {
			String rowProBuy[] = {p.getProduct().getIdProduct(), p.getNameProduct(), p.getAmount()+"", p.getPrice()+"", p.getMoney()+""};
			data_ModelProductBuy.addRow(rowProBuy);
		}
	}
